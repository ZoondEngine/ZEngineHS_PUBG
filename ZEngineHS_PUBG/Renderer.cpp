...
Renderer::Renderer(ID3D11Device * device) : mDevice(device)
{
...
HRESULT Renderer::GenerateShader(ID3D11PixelShader** pS, float r, float g, float b, ID3D10Blob** outBlob)
{
	ID3D10Blob* pBlob;
...
HRESULT Renderer::GenerateShader(ID3D11VertexShader** vS, float r, float g, float b, ID3D10Blob** outBlob)
{
	ID3D10Blob* pBlob;
	char szPixelShader[1000];
	sprintf_s(szPixelShader, d3dcode, r, g, b);
...
void Renderer::AddVertex(Vertex& v, D3D11_PRIMITIVE_TOPOLOGY topology)
{
...
void Renderer::AddVerticies(std::vector<Vertex> v, D3D11_PRIMITIVE_TOPOLOGY topology)
{
...
void Renderer::Line(float x, float y, float j, float k, const Color &color)
{
	std::vector<Vertex> v
	{
		{ x, y, 0.0f, color },
...
void Renderer::Line(Classes::FVector2D origin, Classes::FVector2D dest, Color col)
{
	Line(origin.X, origin.Y, dest.X, dest.Y, col);
}

void Renderer::Box(float x, float y, float j, float k, const Color& color)
{
	Line(x, y, j, y, color);
	Line(j, y, j, k, color);
	Line(j, k, x, k, color);
...
void Renderer::Clear(const Vec4& rect, const Color& color)
{
	std::vector<Vertex> v
	{
...
void Renderer::Text(int x, int y, const std::wstring& text, const Color& color, uint32_t flags, float fontSize)
{
...
void Renderer::WorldLine(Classes::FVector origin, Classes::FVector dest, Color col)
{
...
void Renderer::Box3D(Classes::FVector origin, Classes::FVector extends, Color col)
{
	//origin -= extends / 2.f; Непонял что это такое вообще. Аля недоделанный оператор
...
void Renderer::Plus(float x, float y, float size, Color col)
{
